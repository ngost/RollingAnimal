...
public class StartManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        //shield item using check
        if (StaticInfoManager.shieldEnable)
        {
            actor.transform.Find("Shield").GetComponent<MeshRenderer>().enabled = true;
        }

        if (StaticInfoManager.feverEnable)
        {
            StaticInfoManager.needForFever = 35;
        }
        else
        {
            StaticInfoManager.needForFever = 70;
        }


        try
        {
            actor.transform.position = StaticInfoManager.last_checkpoint;
        }
        catch (UnassignedReferenceException e)
        {
            StaticInfoManager.last_checkpoint = new Vector3(0f, 1f, 0f);
            actor.transform.position = new Vector3(actor.transform.position.x,actor.transform.position.y, StaticInfoManager.last_checkpoint.z);
        }
